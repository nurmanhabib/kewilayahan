...
class KewilayahanLumenServiceProvider extends ServiceProvider
{
    ...
    protected function registerConfig()
    {
        ...
        $this->publishes([
            __DIR__.'/../config/config.php' => $this->app->getConfigurationPath('kewilayahan.php'),
        ], 'config');
    }
    ...
    protected function registerRoute()
    {
        ...
        if ($api_enable) {
            $groups = [
                ...
                'prefix'    => $this->app['config']->get('kewilayahan.api.path', 'api/v1/kewilayahan.json'),
            ];

            $router = $this->app;
            
            $this->app->group($groups, function () use ($router) {
